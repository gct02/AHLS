...
from dataset_analysis.plot_data_metrics import organize_data
...
def get_most_common_directives(
    ...
    for solution in solutions_to_search:
        solution_data_json = Path(f'{dataset_path}/{benchmark_name}/{solution}/{solution}_data.json')
    ...
    directives_sorted = sorted(directives_count.items(), key=lambda x: x[1], reverse=True)
