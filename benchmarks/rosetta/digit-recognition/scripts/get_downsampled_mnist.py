...
# Save the dataset
with open('downsampled_dataset/downsampled_mnist_train.dat', 'wb') as f:
    for image in x_train_downscaled:
        for i in range(4):
            f.write(struct.pack('Q', image[i]))

with open('downsampled_dataset/downsampled_mnist_test.dat', 'wb') as f:
    for image in x_test_downscaled:
        for i in range(4):
            f.write(struct.pack('Q', image[i]))

with open('downsampled_dataset/downsampled_mnist_train_labels.dat', 'wb') as f:
    for label in y_train:
        f.write(label)

with open('downsampled_dataset/downsampled_mnist_test_labels.dat', 'wb') as f:
    for label in y_test:
        f.write(label)