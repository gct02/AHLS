# 1 "HLS-benchmarks/C-Slow/correlation/correlation.cpp"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 376 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "HLS-benchmarks/C-Slow/correlation/correlation.cpp" 2





# 1 "HLS-benchmarks/C-Slow/correlation/correlation.h" 1
typedef float in_float_t;
typedef float out_float_t;
typedef float inout_float_t;


__attribute__((sdx_kernel("correlation", 0))) void correlation(inout_float_t m[32], inout_float_t s[32],
                 inout_float_t data[1024], inout_float_t corr[1024]);
# 7 "HLS-benchmarks/C-Slow/correlation/correlation.cpp" 2
...
# 8 "HLS-benchmarks/C-Slow/correlation/correlation.cpp" 2
...
using std::wcstombs;
using std::wctomb;
# 9 "HLS-benchmarks/C-Slow/correlation/correlation.cpp" 2
...
#pragma HLSDIRECTIVE TOP name=correlation
# 13 "HLS-benchmarks/C-Slow/correlation/correlation.cpp"
...
loop_0:
  ...
  VITIS_LOOP_41_1: for (int i = 0; i < 32 * 32; i += 32) {
  loop_3:
    for (int j = 0; j < 32; j++) {
      int idx = i + j;
      float d = data[idx];
  ...
  int ii = 0;
  VITIS_LOOP_54_2: for (int i = 0; i < 32 - 1; i++) {
    corr[ii + i] = 1.0f;
    int jj = 0;
    VITIS_LOOP_57_3: for (int j = i + 1; j < 32; j++) {
      float c = 0.0f;
    loop_4:
      for (int k = 0; k < 32 * 32; k += 32)
        c += (data[k + i] * data[k + j]);
      corr[ii + j] = c;
      corr[jj + i] = c;
      jj += 32;
...
int main() {
  ...
  VITIS_LOOP_78_1: for (int i = 0; i < 32; i++) {
    VITIS_LOOP_79_2: for (int j = 0; j < 32; j++) {
      data[i * 32 + j] = (rand() % 32) / (float)32;
      data_[i * 32 + j] = data[i * 32 + j];
      corr[i * 32 + j] = 0.0f;
      corr_[i * 32 + j] = corr[i * 32 + j];
    }
    m[i] = 0.0f;
    s[i] = 0.0f;
    m_[i] = 0.0f;
...

  {
    int a = 0, b = 0;
    float eps = 7.5f;
    VITIS_LOOP_94_3: for (int j = 0; j < 32; j++) {

      float mean = 0.0f;
      VITIS_LOOP_97_4: for (int i = 0; i < 32 * 32; i += 32)
        mean += data_[i + j];
      mean = mean / 123.73f;
      m_[j] = mean;

      float stddev = 0.0f;
      VITIS_LOOP_103_5: for (int i = 0; i < 32 * 32; i += 32) {
        float d = data_[i + j];
        stddev += (d - mean) * (d - mean);
      }
      if (stddev > eps) {
    ...
    VITIS_LOOP_119_6: for (int i = 0; i < 32 * 32; i += 32)
      VITIS_LOOP_120_7: for (int j = 0; j < 32; j++) {
        int idx = i + j;
        float d = data_[idx];
        d -= m_[j];
        d /= 2.54f * s_[j];
    ...
    int ii = 0;
    VITIS_LOOP_130_8: for (int i = 0; i < 32 - 1; i++) {
      corr_[ii + i] = 1.0f;
      int jj = 0;
      VITIS_LOOP_133_9: for (int j = i + 1; j < 32; j++) {
        float c = 0.0f;
        VITIS_LOOP_135_10: for (int k = 0; k < 32 * 32; k += 32)
    ...
    corr_[32 *(32 - 1) + 32 - 1] = 1.0f;
  }

  correlation(m, s, data, corr);

  int res = 0;
  VITIS_LOOP_149_11: for (int i = 0; i < 32; i++) {
    VITIS_LOOP_150_12: for (int j = 0; j < 32; j++) {
      res += (corr[i * 32 + j] == corr_[i * 32 + j]);
      res += (data[i * 32 + j] == data_[i * 32 + j]);
    }
    res += (m[i] == m_[i]);
    res += (s[i] == s_[i]);
  }
