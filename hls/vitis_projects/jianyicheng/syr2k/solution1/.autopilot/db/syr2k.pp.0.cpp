...
# 8 "HLS-benchmarks/C-Slow/syr2k/syr2k.cpp"
  ...
  VITIS_LOOP_11_1: for (int i = 0; i < 32; i++) {
    int jj = 0;
    VITIS_LOOP_13_2: for (int j = 0; j <= i; j++) {
      int idx = ii + j;
...
int main() {
  ...
  VITIS_LOOP_31_1: for (int i = 0; i < 32; i++) {
    VITIS_LOOP_32_2: for (int j = 0; j < 32; j++) {
      A[i * 32 + j] = 0.1f;
      A_[i * 32 + j] = A[i * 32 + j];
      B[i * 32 + j] = 0.1f;
      B_[i * 32 + j] = B[i * 32 + j];
...

  {
    float alpha = 0.73f, beta = 1.96f;
    int ii = 0;
    VITIS_LOOP_45_3: for (int i = 0; i < 32; i++) {
      int jj = 0;
      VITIS_LOOP_47_4: for (int j = 0; j <= i; j++) {
        float sum = C_[ii + j] * beta;
        VITIS_LOOP_49_5: for (int k = 0; k < 32; k++)
          sum +=
