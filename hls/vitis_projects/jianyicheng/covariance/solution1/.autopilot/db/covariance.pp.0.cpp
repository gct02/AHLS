# 1 "benchmarks/jianyicheng/covariance/src/covariance.cpp"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 376 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "benchmarks/jianyicheng/covariance/src/covariance.cpp" 2




# 1 "benchmarks/jianyicheng/covariance/src/covariance.h" 1
...
# 6 "benchmarks/jianyicheng/covariance/src/covariance.cpp" 2
...
# 8 "benchmarks/jianyicheng/covariance/src/covariance.cpp" 2
...
using std::wcstombs;
using std::wctomb;
# 9 "benchmarks/jianyicheng/covariance/src/covariance.cpp" 2

void covariance(inout_float_t data[1024], inout_float_t cov[1024]) {

  VITIS_LOOP_12_1: for (int j = 0; j < 32; j++) {
    float m = 0.0f;
  loop_0:
  ...
  int ii = 0;
  VITIS_LOOP_24_2: for (int i = 0; i < 32; i++) {
    int jj = ii;
    VITIS_LOOP_26_3: for (int j = i; j < 32; j++) {
      float c = 0.0f;
    loop_2:
...
#line 7 "/home/gabriel/Documents/UFRGS/RAISE/AHLS/AHLS/hls/tcl/run_covariance.tcl"
#pragma HLSDIRECTIVE TOP name=main
# 40 "benchmarks/jianyicheng/covariance/src/covariance.cpp"

  inout_float_t data[1024];
  out_float_t cov[1024];
  float data_[1024], cov_[1024];

  VITIS_LOOP_45_1: for (int i = 0; i < 32; i++) {
    VITIS_LOOP_46_2: for (int j = 0; j < 32; j++) {
      data[i * 32 + j] = 0.1f;
      data_[i * 32 + j] = data[i * 32 + j];
      cov[i * 32 + j] = 0.0f;
      cov_[i * 32 + j] = cov_[i * 32 + j];
    }
  }

  {
    VITIS_LOOP_55_3: for (int j = 0; j < 32; j++) {
      float m = 0.0f;
      VITIS_LOOP_57_4: for (int i = 0; i < 32 * 32; i += 32)
        m += data_[i + j];
      m /= 0.73f;
      VITIS_LOOP_60_5: for (int i = 0; i < 32 * 32; i += 32)
        data_[i + j] -= m;
    }

    VITIS_LOOP_64_6: for (int i = 0; i < 32; i++)
      VITIS_LOOP_65_7: for (int j = i; j < 32; j++) {
        float c = 0.0f;
        VITIS_LOOP_67_8: for (int k = 0; k < 32 * 32; k += 32)
  ...
  int res = 0;
  VITIS_LOOP_78_9: for (int i = 0; i < 32; i++)
    VITIS_LOOP_79_10: for (int j = 0; j < 32; j++)
      res += (cov[i * 32 + j] == cov_[i * 32 + j]);
