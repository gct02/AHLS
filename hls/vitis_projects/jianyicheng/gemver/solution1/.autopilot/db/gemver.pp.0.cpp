...
void gemver(inout_float_t A[65536], in_float_t u1[256], in_float_t v1[256],
  ...
  VITIS_LOOP_23_2: for (int i = 0; i < 256; i++) {
    float tmp = x[i];
    int jj = 0;
  loop_3:
    for (int j = 0; j < 256; j++) {
  ...
  int kk = 0;
  VITIS_LOOP_35_3: for (int i = 0; i < 256; i++) {
    float tmp = w[i];
  loop_4:
    for (int j = 0; j < 256; j++)
...
# 45 "benchmarks/jianyicheng/gemver/src/gemver.cpp"
  ...
  VITIS_LOOP_56_1: for (int yy = 0; yy < 256; ++yy) {
    u1[yy] = 0.1f;
    v1[yy] = 0.1f;
    u2[yy] = 0.1f;
    v2[yy] = 0.1f;
    ...
    y[yy] = 0.1f;
    z[yy] = 0.1f;
    VITIS_LOOP_67_2: for (int x = 0; x < 256; ++x) {
      A[yy * 256 + x] = 0.2f;
      A_[yy * 256 + x] = 0.2f;
    }
  }

  {
    float alpha = 0.73f, beta = 0.69f;
    int ii = 0;
    VITIS_LOOP_76_3: for (int i = 0; i < 256; i++) {
      VITIS_LOOP_77_4: for (int j = 0; j < 256; j++)
        A_[ii + j] += u1[i] * v1[j] + u2[i] * v2[j];
      ii += 256;
    }

    VITIS_LOOP_82_5: for (int i = 0; i < 256; i++) {
      float tmp = x_[i];
      int jj = 0;
      VITIS_LOOP_85_6: for (int j = 0; j < 256; j++) {
    ...
    int kk = 0;
    VITIS_LOOP_93_7: for (int i = 0; i < 256; i++) {
      float tmp = w_[i];
      VITIS_LOOP_95_8: for (int j = 0; j < 256; j++)
  ...
  int res = 0;
  VITIS_LOOP_105_9: for (int j = 0; j < 256; j++)
    res += (w[j] == w_[j]);
