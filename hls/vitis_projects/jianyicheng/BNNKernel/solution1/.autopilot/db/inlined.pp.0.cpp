# 1 "benchmarks/jianyicheng/BNNKernel/src/inlined.cpp"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 376 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "benchmarks/jianyicheng/BNNKernel/src/inlined.cpp" 2
# 10 "benchmarks/jianyicheng/BNNKernel/src/inlined.cpp"
...
# 11 "benchmarks/jianyicheng/BNNKernel/src/inlined.cpp" 2

void inlined(int addr_in[10000], int alpha, int w[10000], int addr_out[10000], int data[10000], int in[10000], int mean[10000]){
  int lut, m,i,j,k, x, y, z, temp;

  VITIS_LOOP_15_1: for (i=0; i<100; i++){
 VITIS_LOOP_16_2: for (j=0;j<100;j++){
  x = i*100+j;
  lut=in[x]^w[x];
  data[x] += lut*alpha;
 }
 ...
 if (i==99){
    VITIS_LOOP_23_3: for(k=0; k<100; k++){
    y=i*100+k;
    m=mean[y];
    temp = data[y];
    if (temp > 0)
