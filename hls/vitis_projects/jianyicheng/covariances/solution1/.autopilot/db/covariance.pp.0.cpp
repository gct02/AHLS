# 1 "HLS-benchmarks/C-Slow/covariance/covariance.cpp"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 376 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "HLS-benchmarks/C-Slow/covariance/covariance.cpp" 2




# 1 "HLS-benchmarks/C-Slow/covariance/covariance.h" 1
typedef float in_float_t;
typedef float out_float_t;
typedef float inout_float_t;


__attribute__((sdx_kernel("covariance", 0))) void covariance(inout_float_t data[1024], inout_float_t cov[1024]);
# 6 "HLS-benchmarks/C-Slow/covariance/covariance.cpp" 2
...
using std::wcstombs;
using std::wctomb;
# 7 "HLS-benchmarks/C-Slow/covariance/covariance.cpp" 2

__attribute__((sdx_kernel("covariance", 0))) void covariance(inout_float_t data[1024], inout_float_t cov[1024]) {
#line 16 "/home/gabriel/Documents/UFRGS/RAISE/ahls_resource_estimation/run_hls.tcl"
#pragma HLSDIRECTIVE TOP name=covariance
# 8 "HLS-benchmarks/C-Slow/covariance/covariance.cpp"


  VITIS_LOOP_10_1: for (int j = 0; j < 32; j++) {
    float m = 0.0f;
  loop_0:
  ...
  int ii = 0;
  VITIS_LOOP_22_2: for (int i = 0; i < 32; i++) {
    int jj = ii;
    VITIS_LOOP_24_3: for (int j = i; j < 32; j++) {
      float c = 0.0f;
    loop_2:
...
int main() {
  ...
  VITIS_LOOP_43_1: for (int i = 0; i < 32; i++) {
    VITIS_LOOP_44_2: for (int j = 0; j < 32; j++) {
      data[i * 32 + j] = 0.1f;
  ...
  {
    VITIS_LOOP_53_3: for (int j = 0; j < 32; j++) {
      float m = 0.0f;
      VITIS_LOOP_55_4: for (int i = 0; i < 32 * 32; i += 32)
        m += data_[i + j];
      m /= 0.73f;
      VITIS_LOOP_58_5: for (int i = 0; i < 32 * 32; i += 32)
        data_[i + j] -= m;
    }

    VITIS_LOOP_62_6: for (int i = 0; i < 32; i++)
      VITIS_LOOP_63_7: for (int j = i; j < 32; j++) {
        float c = 0.0f;
        VITIS_LOOP_65_8: for (int k = 0; k < 32 * 32; k += 32)
  ...
  int res = 0;
  VITIS_LOOP_76_9: for (int i = 0; i < 32; i++)
    VITIS_LOOP_77_10: for (int j = 0; j < 32; j++)
