# 1 "HLS-benchmarks/C-Slow/gesummv/gesummv.cpp"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 376 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "HLS-benchmarks/C-Slow/gesummv/gesummv.cpp" 2




# 1 "HLS-benchmarks/C-Slow/gesummv/gesummv.h" 1
...
__attribute__((sdx_kernel("gesummv", 0))) void gesummv(in_float_t A[65536], in_float_t B[65536], out_float_t tmp[256],
             out_float_t y[256], in_float_t x[256]);
# 6 "HLS-benchmarks/C-Slow/gesummv/gesummv.cpp" 2
...
using std::wcstombs;
using std::wctomb;
# 7 "HLS-benchmarks/C-Slow/gesummv/gesummv.cpp" 2

__attribute__((sdx_kernel("gesummv", 0))) void gesummv(in_float_t A[65536], in_float_t B[65536], out_float_t tmp[256],
             out_float_t y[256], in_float_t x[256]) {
#line 16 "/home/gabriel/Documents/UFRGS/RAISE/ahls_resource_estimation/run_hls.tcl"
#pragma HLSDIRECTIVE TOP name=gesummv
# 9 "HLS-benchmarks/C-Slow/gesummv/gesummv.cpp"
...
int main(void) {
  ...
  VITIS_LOOP_38_1: for (int yy = 0; yy < 256; ++yy) {
    x[yy] = 0.1f;
    y[yy] = 0.1f;
    tmp[yy] = 0.1f;
    ...
    VITIS_LOOP_46_2: for (int x = 0; x < 256; ++x) {
      A[yy * 256 + x] = 0.2f;
...

  {
    ...
    int ii = 0;
    VITIS_LOOP_56_3: for (int i = 0; i < 256; i++) {
      float t_tmp = 0.0f;
      float t_y = 0.0f;

      VITIS_LOOP_60_4: for (int j = 0; j < 256; j++) {
        float t_x = x[j];
        t_tmp = A[ii + j] * t_x + t_tmp;
        t_y = B[ii + j] * t_x + t_y;
      }
