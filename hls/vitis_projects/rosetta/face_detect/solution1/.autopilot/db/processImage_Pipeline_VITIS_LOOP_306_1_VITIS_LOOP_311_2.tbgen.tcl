set moduleName processImage_Pipeline_VITIS_LOOP_306_1_VITIS_LOOP_311_2
set isTopModule 0
set isCombinational 0
set isDatapathOnly 0
set isPipelined 1
set pipeline_type none
set FunctionProtocol ap_ctrl_hs
set isOneStateSeq 0
set ProfileFlag 0
set StallSigGenFlag 0
set isEnableWaveformDebug 1
set hasInterrupt 0
set DLRegFirstOffset 0
set DLRegItemOffset 0
set C_modelName {processImage_Pipeline_VITIS_LOOP_306_1_VITIS_LOOP_311_2}
set C_modelType { void 0 }
set C_modelArgList {
	{ sum_col int 32 regular  }
	{ bound int 64 regular  }
	{ IMG1_data int 8 regular {array 76800 { 1 3 } 1 1 }  }
	{ SUM1_data int 32 regular {array 76800 { 2 3 3 3 3 3 3 3 3 3 3 3 3 3 3 3 3 } 1 1 }  }
	{ SQSUM1_data int 32 regular {array 76800 { 2 3 } 1 1 }  }
}
set hasAXIMCache 0
set AXIMCacheInstList { }
set C_modelArgMapList {[ 
	{ "Name" : "sum_col", "interface" : "wire", "bitwidth" : 32, "direction" : "READONLY"} , 
 	{ "Name" : "bound", "interface" : "wire", "bitwidth" : 64, "direction" : "READONLY"} , 
 	{ "Name" : "IMG1_data", "interface" : "memory", "bitwidth" : 8, "direction" : "READONLY"} , 
 	{ "Name" : "SUM1_data", "interface" : "memory", "bitwidth" : 32, "direction" : "READWRITE"} , 
 	{ "Name" : "SQSUM1_data", "interface" : "memory", "bitwidth" : 32, "direction" : "READWRITE"} ]}
# RTL Port declarations: 
set portNum 21
set portList { 
	{ ap_clk sc_in sc_logic 1 clock -1 } 
	{ ap_rst sc_in sc_logic 1 reset -1 active_high_sync } 
	{ ap_start sc_in sc_logic 1 start -1 } 
	{ ap_done sc_out sc_logic 1 predone -1 } 
	{ ap_idle sc_out sc_logic 1 done -1 } 
	{ ap_ready sc_out sc_logic 1 ready -1 } 
	{ sum_col sc_in sc_lv 32 signal 0 } 
	{ bound sc_in sc_lv 64 signal 1 } 
	{ IMG1_data_address0 sc_out sc_lv 17 signal 2 } 
	{ IMG1_data_ce0 sc_out sc_logic 1 signal 2 } 
	{ IMG1_data_q0 sc_in sc_lv 8 signal 2 } 
	{ SUM1_data_address0 sc_out sc_lv 17 signal 3 } 
	{ SUM1_data_ce0 sc_out sc_logic 1 signal 3 } 
	{ SUM1_data_we0 sc_out sc_logic 1 signal 3 } 
	{ SUM1_data_d0 sc_out sc_lv 32 signal 3 } 
	{ SUM1_data_q0 sc_in sc_lv 32 signal 3 } 
	{ SQSUM1_data_address0 sc_out sc_lv 17 signal 4 } 
	{ SQSUM1_data_ce0 sc_out sc_logic 1 signal 4 } 
	{ SQSUM1_data_we0 sc_out sc_logic 1 signal 4 } 
	{ SQSUM1_data_d0 sc_out sc_lv 32 signal 4 } 
	{ SQSUM1_data_q0 sc_in sc_lv 32 signal 4 } 
}
set NewPortList {[ 
	{ "name": "ap_clk", "direction": "in", "datatype": "sc_logic", "bitwidth":1, "type": "clock", "bundle":{"name": "ap_clk", "role": "default" }} , 
 	{ "name": "ap_rst", "direction": "in", "datatype": "sc_logic", "bitwidth":1, "type": "reset", "bundle":{"name": "ap_rst", "role": "default" }} , 
 	{ "name": "ap_start", "direction": "in", "datatype": "sc_logic", "bitwidth":1, "type": "start", "bundle":{"name": "ap_start", "role": "default" }} , 
 	{ "name": "ap_done", "direction": "out", "datatype": "sc_logic", "bitwidth":1, "type": "predone", "bundle":{"name": "ap_done", "role": "default" }} , 
 	{ "name": "ap_idle", "direction": "out", "datatype": "sc_logic", "bitwidth":1, "type": "done", "bundle":{"name": "ap_idle", "role": "default" }} , 
 	{ "name": "ap_ready", "direction": "out", "datatype": "sc_logic", "bitwidth":1, "type": "ready", "bundle":{"name": "ap_ready", "role": "default" }} , 
 	{ "name": "sum_col", "direction": "in", "datatype": "sc_lv", "bitwidth":32, "type": "signal", "bundle":{"name": "sum_col", "role": "default" }} , 
 	{ "name": "bound", "direction": "in", "datatype": "sc_lv", "bitwidth":64, "type": "signal", "bundle":{"name": "bound", "role": "default" }} , 
 	{ "name": "IMG1_data_address0", "direction": "out", "datatype": "sc_lv", "bitwidth":17, "type": "signal", "bundle":{"name": "IMG1_data", "role": "address0" }} , 
 	{ "name": "IMG1_data_ce0", "direction": "out", "datatype": "sc_logic", "bitwidth":1, "type": "signal", "bundle":{"name": "IMG1_data", "role": "ce0" }} , 
 	{ "name": "IMG1_data_q0", "direction": "in", "datatype": "sc_lv", "bitwidth":8, "type": "signal", "bundle":{"name": "IMG1_data", "role": "q0" }} , 
 	{ "name": "SUM1_data_address0", "direction": "out", "datatype": "sc_lv", "bitwidth":17, "type": "signal", "bundle":{"name": "SUM1_data", "role": "address0" }} , 
 	{ "name": "SUM1_data_ce0", "direction": "out", "datatype": "sc_logic", "bitwidth":1, "type": "signal", "bundle":{"name": "SUM1_data", "role": "ce0" }} , 
 	{ "name": "SUM1_data_we0", "direction": "out", "datatype": "sc_logic", "bitwidth":1, "type": "signal", "bundle":{"name": "SUM1_data", "role": "we0" }} , 
 	{ "name": "SUM1_data_d0", "direction": "out", "datatype": "sc_lv", "bitwidth":32, "type": "signal", "bundle":{"name": "SUM1_data", "role": "d0" }} , 
 	{ "name": "SUM1_data_q0", "direction": "in", "datatype": "sc_lv", "bitwidth":32, "type": "signal", "bundle":{"name": "SUM1_data", "role": "q0" }} , 
 	{ "name": "SQSUM1_data_address0", "direction": "out", "datatype": "sc_lv", "bitwidth":17, "type": "signal", "bundle":{"name": "SQSUM1_data", "role": "address0" }} , 
 	{ "name": "SQSUM1_data_ce0", "direction": "out", "datatype": "sc_logic", "bitwidth":1, "type": "signal", "bundle":{"name": "SQSUM1_data", "role": "ce0" }} , 
 	{ "name": "SQSUM1_data_we0", "direction": "out", "datatype": "sc_logic", "bitwidth":1, "type": "signal", "bundle":{"name": "SQSUM1_data", "role": "we0" }} , 
 	{ "name": "SQSUM1_data_d0", "direction": "out", "datatype": "sc_lv", "bitwidth":32, "type": "signal", "bundle":{"name": "SQSUM1_data", "role": "d0" }} , 
 	{ "name": "SQSUM1_data_q0", "direction": "in", "datatype": "sc_lv", "bitwidth":32, "type": "signal", "bundle":{"name": "SQSUM1_data", "role": "q0" }}  ]}

set RtlHierarchyInfo {[
	{"ID" : "0", "Level" : "0", "Path" : "`AUTOTB_DUT_INST", "Parent" : "", "Child" : ["1", "2"],
		"CDFG" : "processImage_Pipeline_VITIS_LOOP_306_1_VITIS_LOOP_311_2",
		"Protocol" : "ap_ctrl_hs",
		"ControlExist" : "1", "ap_start" : "1", "ap_ready" : "1", "ap_done" : "1", "ap_continue" : "0", "ap_idle" : "1", "real_start" : "0",
		"Pipeline" : "None", "UnalignedPipeline" : "0", "RewindPipeline" : "0", "ProcessNetwork" : "0",
		"II" : "0",
		"VariableLatency" : "1", "ExactLatency" : "-1", "EstimateLatencyMin" : "-1", "EstimateLatencyMax" : "-1",
		"Combinational" : "0",
		"Datapath" : "0",
		"ClockEnable" : "0",
		"HasSubDataflow" : "0",
		"InDataflowNetwork" : "0",
		"HasNonBlockingOperation" : "0",
		"IsBlackBox" : "0",
		"Port" : [
			{"Name" : "sum_col", "Type" : "None", "Direction" : "I"},
			{"Name" : "bound", "Type" : "None", "Direction" : "I"},
			{"Name" : "IMG1_data", "Type" : "Memory", "Direction" : "I"},
			{"Name" : "SUM1_data", "Type" : "Memory", "Direction" : "IO"},
			{"Name" : "SQSUM1_data", "Type" : "Memory", "Direction" : "IO"}],
		"Loop" : [
			{"Name" : "VITIS_LOOP_306_1_VITIS_LOOP_311_2", "PipelineType" : "UPC",
				"LoopDec" : {"FSMBitwidth" : "2", "FirstState" : "ap_ST_fsm_pp0_stage0", "FirstStateIter" : "ap_enable_reg_pp0_iter0", "FirstStateBlock" : "ap_block_pp0_stage0_subdone", "LastState" : "ap_ST_fsm_pp0_stage1", "LastStateIter" : "ap_enable_reg_pp0_iter5", "LastStateBlock" : "ap_block_pp0_stage1_subdone", "QuitState" : "ap_ST_fsm_pp0_stage1", "QuitStateIter" : "ap_enable_reg_pp0_iter5", "QuitStateBlock" : "ap_block_pp0_stage1_subdone", "OneDepthLoop" : "0", "has_ap_ctrl" : "1", "has_continue" : "0"}}]},
	{"ID" : "1", "Level" : "1", "Path" : "`AUTOTB_DUT_INST.mac_muladd_8ns_8ns_32ns_32_4_1_U11", "Parent" : "0"},
	{"ID" : "2", "Level" : "1", "Path" : "`AUTOTB_DUT_INST.flow_control_loop_pipe_sequential_init_U", "Parent" : "0"}]}


set ArgLastReadFirstWriteLatency {
	processImage_Pipeline_VITIS_LOOP_306_1_VITIS_LOOP_311_2 {
		sum_col {Type I LastRead 0 FirstWrite -1}
		bound {Type I LastRead 0 FirstWrite -1}
		IMG1_data {Type I LastRead 4 FirstWrite -1}
		SUM1_data {Type IO LastRead 5 FirstWrite 8}
		SQSUM1_data {Type IO LastRead 8 FirstWrite 11}}}

set hasDtUnsupportedChannel 0

set PerformanceInfo {[
	{"Name" : "Latency", "Min" : "-1", "Max" : "-1"}
	, {"Name" : "Interval", "Min" : "-1", "Max" : "-1"}
]}

set PipelineEnableSignalInfo {[
	{"Pipeline" : "0", "EnableSignal" : "ap_enable_pp0"}
]}

set Spec2ImplPortList { 
	sum_col { ap_none {  { sum_col in_data 0 32 } } }
	bound { ap_none {  { bound in_data 0 64 } } }
	IMG1_data { ap_memory {  { IMG1_data_address0 mem_address 1 17 }  { IMG1_data_ce0 mem_ce 1 1 }  { IMG1_data_q0 mem_dout 0 8 } } }
	SUM1_data { ap_memory {  { SUM1_data_address0 mem_address 1 17 }  { SUM1_data_ce0 mem_ce 1 1 }  { SUM1_data_we0 mem_we 1 1 }  { SUM1_data_d0 mem_din 1 32 }  { SUM1_data_q0 mem_dout 0 32 } } }
	SQSUM1_data { ap_memory {  { SQSUM1_data_address0 mem_address 1 17 }  { SQSUM1_data_ce0 mem_ce 1 1 }  { SQSUM1_data_we0 mem_we 1 1 }  { SQSUM1_data_d0 mem_din 1 32 }  { SQSUM1_data_q0 mem_dout 0 32 } } }
}
