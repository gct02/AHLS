...
#pragma HLS LOOP_TRIPCOUNT min=160 max=160 avg=160
 temp = gsm_abs(s[k]);
    ...
    if (scalauto > 0 && scalauto <= 4) {
        n = scalauto;
        VITIS_LOOP_64_1: for (k = 0; k <= 159; k++) {
    ...
    VITIS_LOOP_124_3: for (i = 8; i <= 159; i++) {
        sl = *++sp;
        L_ACF[0] += ((longword)sl * sp[-(0)]);;
        L_ACF[1] += ((longword)sl * sp[-(1)]);;
        L_ACF[2] += ((longword)sl * sp[-(2)]);;
    ...
    VITIS_LOOP_137_4: for (k = 8; k >= 0; k--) {
        L_ACF[k] <<= 1;
    }


    if (scalauto > 0) {
        VITIS_LOOP_143_5: for (k = 159; k >= 0; k--) {
            *s++ <<= scalauto;
        }
    }
}
...
#pragma HLSDIRECTIVE PIPELINE off=true
# 181 "data/benchmarks/gsm/gsm_lpc.c"
...
#pragma HLSDIRECTIVE PIPELINE off=true
# 187 "data/benchmarks/gsm/gsm_lpc.c"
...
#pragma HLSDIRECTIVE PIPELINE off=true
# 194 "data/benchmarks/gsm/gsm_lpc.c"
...
#pragma HLSDIRECTIVE PIPELINE off=true
# 200 "data/benchmarks/gsm/gsm_lpc.c"
...
#pragma HLSDIRECTIVE PIPELINE off=true
# 219 "data/benchmarks/gsm/gsm_lpc.c"
...
void Transformation_to_Log_Area_Ratios(register word *r )
{
    ...
    VITIS_LOOP_237_1: for (i = 1; i <= 8; i++, r++) {
        temp = *r;
        temp = gsm_abs(temp);
...
{
    register word temp;
# 277 "data/benchmarks/gsm/gsm_lpc.c"
