...
void integralImages(
  int height, int width, unsigned char Data[IMAGE_HEIGHT][IMAGE_WIDTH], 
  int Sum[IMAGE_HEIGHT][IMAGE_WIDTH], int Sqsum[IMAGE_HEIGHT][IMAGE_WIDTH]
);

void imageScaler(
  int src_height, int src_width, unsigned char Data[IMAGE_HEIGHT][IMAGE_WIDTH],
  int dest_height, int dest_width, unsigned char IMG1_data[IMAGE_HEIGHT][IMAGE_WIDTH]
);

void processImage( 
  float factor, int sum_row, int sum_col, int *AllCandidates_x, int *AllCandidates_y,
  int *AllCandidates_w, int *AllCandidates_h, int *AllCandidates_size,
  unsigned char IMG1_data[IMAGE_HEIGHT][IMAGE_WIDTH], MySize winSize
);

int cascadeClassifier(
  int SUM1_data[IMAGE_HEIGHT][IMAGE_WIDTH], int SQSUM1_data[IMAGE_HEIGHT][IMAGE_WIDTH], 
  MyPoint pt
); 

int weakClassifier(int stddev, int coord[12], int haar_counter, int w_id);

void groupRectangles(MyRect *rectList, int *rectList_size, int groupThreshold, float eps);
...
)
{
  ...
  *result_size = 0;
  ...
  for ( i = 0; i < 12; i++ )
  {
    ...
    processImage(factor, height, width, result_x, result_y, result_w, result_h, result_size, IMG1_data, winSize); 
...
)
{
  ...
  Pixely: 
  for( y = 0; y < sum_row - WINDOW_SIZE + 1; y++ ){
    #pragma HLS TRIPCOUNT min=2 max=176
    Pixelx: 
    for ( x = 0; x < sum_col - WINDOW_SIZE + 1; x++ ){
      #pragma HLS TRIPCOUNT min=11 max=242
      p.x = x;
      p.y = y;
...
)
{
  ...
  for( y = 0; y < height; y++)
  {
    #pragma HLS LOOP_TRIPCOUNT min=26 max=200
    s = 0;
    sq = 0;
    ...
    for( x = 0; x < width; x ++)
    {
      #pragma HLS LOOP_TRIPCOUNT min=35 max=266
      it = Data[y][x];
