...
def process_ir(
    ir_src: Path, ir_dst: Path, directives: Path,
    out_md: Path, out_loop_hierarchy: Path
) -> None:
    """
    ...
    base-transform: prepare the IR removing 'llvm.dbg', 'llvm.lifetime' and 
        '_ssdm_Spec.* intrinsics, implementing 'part.select' and 'part.set',
        and assigning a unique ID to each operation, function, basic block, and
        global value;
    update-md: update the metadata of entities in the IR;
    set-hls-md: set metadata about the solution directives to entities in the IR;
    ...
    out_loop_hierarchy: Path
        Path to the file where the loop hierarchy extracted from the IR will be written
    ...
    try:
        ...
        run_opt(tmp2, tmp1, "-base-transform")
        run_opt(tmp1, tmp2, "-update-md")
        run_opt(tmp2, tmp1, f"-set-hls-md -directive {directives.as_posix()}")
        ...
        subprocess.check_output(
            f"{OPT} -load {DSE_LIB} -loop-hierarchy -out-lh {out_loop_hierarchy.as_posix()} < {tmp2.as_posix()};", 
            shell=True, stderr=subprocess.STDOUT
        )
        run_opt(tmp2, ir_dst, "-cleanup-extracted-params")
    except subprocess.CalledProcessError as e:
...
def main(args: Dict[str, str]):
    ...
    for benchmark in benchmarks:
        ...
        for solution in solutions:
            ...
            output_loop_hierarchy_path = output_instance_folder / "loop_hierarchy.txt"

            try:
                process_ir(
                    ir, ir_mod, directives_tcl_path, 
                    output_md_path, output_loop_hierarchy_path
                )
            except subprocess.CalledProcessError:
                print(f"Error processing {ir}")
                shutil.rmtree(output_instance_folder)
                continue
