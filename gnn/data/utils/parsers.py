...
def extract_metrics(solution_dir, filtered=False) -> Dict[str, Union[Report, Number]]:
...
def parse_tcl_directives(tcl_path) -> List[Tuple[str, Dict[str, str]]]:
...
def _parse_directive_cmd(directive_cmd: str) -> Tuple[str, Dict[str, str]]:
    cmd = directive_cmd.split(' ')[0].split('set_directive_')[1]
    args = directive_cmd.split(' ')[1:]
    parsed_args = _parse_directive_cmd_options(args)
    return cmd, parsed_args


def _parse_directive_cmd_options(args: List[str]) -> Dict[str, str]:
    parsed_args = {}
    is_loc_parsed = False
    i = 0
