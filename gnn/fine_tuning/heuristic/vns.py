...
from gnn.fine_tuning.heuristic.domain import (
    PARAM_GROUPS,
    LEARNING_RATES,
    NUM_LR_CONFIGS,
    NUM_PARAM_GROUPS,
...
from gnn.fine_tuning.utils import (
    set_random_seeds,
    prepare_fine_tuning_data_loader,
    prepare_evaluation_data_loader,
...
available_resources = torch.tensor(
    [AVAILABLE_RESOURCES[r] for r in TARGET_AREA_METRICS],
    dtype=torch.float32,
    device=DEVICE
)
evaluated_configs = {}
...
def vns(
    ...
    for bench_name, bench_info in BENCHMARK_INFO.items():
        ...
        ft_loader = prepare_fine_tuning_data_loader(
