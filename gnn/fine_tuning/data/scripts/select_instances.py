...
if __name__ == "__main__":
    ...
    parser.add_argument("-p", "--processed_benchmark_dir", type=str, default=None, 
    ...
    parser.add_argument("-n", "--num_solutions", type=int, default=20, 
    ...
    if not args.benchmark_dir and not args.processed_benchmark_dir:
        print("Either benchmark_dir or processed_benchmark_dir must be provided.")
        sys.exit(1)
    ...
    if args.processed_benchmark_dir:
        if not os.path.exists(args.processed_benchmark_dir):
            print(f"Processed benchmark directory {args.processed_benchmark_dir} does not exist.")
            sys.exit(1)
    else:
    ...
    benchmark_name = kernel_info.benchmark_name.upper()
    if benchmark_name not in benchmark_info:
        print(f"Benchmark {benchmark_name} not found in benchmark info.")
        sys.exit(1)

    benchmark_info = benchmark_info[benchmark_name]
