...
from estimators.hgat.utils.cdfg import build_cdfg
    ...
    for i, benchmark in enumerate(benchmarks):
        ...
        for j, solution in enumerate(solutions):
            ...
            subprocess.check_output(f"{OPT} -mem2reg < {ir_temp_1.as_posix()} > {ir_temp_2.as_posix()};",\
                                    stderr=subprocess.STDOUT, shell=True)
            subprocess.check_output(f"{OPT} -load {AHLS_LLVM_LIB} -preprocess-vitis-ir < {ir_temp_2.as_posix()} > {ir_temp_1.as_posix()};",\
                                    stderr=subprocess.STDOUT, shell=True)
            ...
            cdfg = build_cdfg(ir_mod, has_directives=True)
