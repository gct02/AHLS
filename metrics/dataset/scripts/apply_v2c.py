...
from design_metrics.data_stats_parser import *
...
def create_design_subdirs(parent_dir: Path, design_name: str) -> tuple[Path, Path, Path, Path]:
...
if __name__ == "__main__":
    ...
    approx_ir_files, approx_exec_files, data_stats = apply_v2m_to_all_ops(exact_design_ir_dir, exact_data_stats_path, approx_design_ir_dir, approx_design_exec_dir)

    design_name_len = len(design_name)
    for i, approx_exec_file in enumerate(approx_exec_files):
        approx_suffix = approx_exec_file.stem[design_name_len:]
        approx_output_path = approx_design_output_dir / f"{design_name}{input_args_str}{approx_suffix}.txt"
        approx_data_stats_path = approx_design_data_stats_dir / f"{design_name}{input_args_str}{approx_suffix}.txt"

        if run_approx(approx_exec_file, input_exec_args, approx_output_path, approx_data_stats_path, timeout) == APPROX_ERROR:
            continue
        
        # Resource savings estimator features
        # Use all removed (or even all modified) operations instead of just the approximation target?
        # modified_ops, removed_ops, added_ops = get_transformed_ops_stats(data_stats, parse_data_stats_file(approx_data_stats_path))

        opcode = data_stats[i][0]
        is_signed = data_stats[i][1]
        is_fp = data_stats[i][2]
        bitwidth = data_stats[i][3]
        num_occurs = data_stats[i][4]
        num_uses = data_stats[i][9] # change with the actual uses of the operation (it can be done using an embedding model to represent it as a single vector)
        feats = [opcode, is_signed, is_fp, bitwidth, num_uses, num_occurs]

        """
        TODO:
        1. Parse the post-synthesis resource usage report of the exact design.
        2. Synthesize the approximate design using Vivado HLS.
        3. Parse the post-synthesis resource usage report of the approximate design.
        4. Calculate the resource savings.
        5. Create one training/test sample for the resource savings estimator with 'feats' as the input (sample features)
           and the resource savings as the output (sample label).
        """
